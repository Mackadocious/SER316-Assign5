...
public class FishFarm implements Farm, FishSubject, FarmObserver, PredatorObserver {
    ...
    private void checkForBabies() { //creates eggs between two fish of the same type based on probability
        Fish tempFish;
        int tunaCount = 0;
        int salmonCount = 0;
        int codCount = 0;
    ...
    @Override
    public void notifyOfSick() { //notifies all fish on the farm that they have a disease using the observer
        System.out.println("All fish on " + this.name + " have come down with a disease");
        grabber.notifyOfSick();
    }

    @Override
    public void notifyOfWell() { //notifies all fishes on a single farm that the disease has been cured using the observer
    ...
    @Override
    public void checkDeath() { //checks to see if any fishes are too old and die, or if fishes have become diseased using the observer.
        int diedCounter = 0;
        for (int i = 0; i < fishes.size(); i++) {
        ...
        System.out.println(diedCounter + " fish on farm " + this.name + " have died of it's sickness \n" +
        ...
        System.out.println(diedCounter + " fish on farm " + this.name + " have died of it's old age \n " +
